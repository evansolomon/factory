...
import { grillPrompt } from './prompts.ts'

// Interactive intake grill. Seeded with the raw intent, an agent interrogates it
...
export type GrillResult = { intent: string; verify: string | null }
type GrillOpts = {
  root: string
  agent: Agent
  hooks: Hooks
...
function parseGrill(text: string): Parsed {
...
export async function grill(opts: GrillOpts): Promise<GrillResult | null> {
  log.step('grill — refining into a spec')
  log.info(
  ...
  let proposed: GrillResult | null = null
  ...
  try {
    ...
    log.warn('grill: reached the turn limit — finalizing')
    return proposed ?? (await finalize(opts, turns, tally))
  } finally {
    rl.close()
    await emit(opts.root, opts.hooks, 'attention', { state: 'none' })
...
async function finalize(
  ...
  if (!parsed.ready || !parsed.spec) {
    log.warn('grill: could not synthesize a spec — keeping your original intent')
